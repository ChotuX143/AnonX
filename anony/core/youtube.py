...
# ðŸŸ¢ Primary API (Inflex)
INFLEX_API_URL = os.environ.get("INFLEX_API_URL", "https://teaminflex.xyz")
INFLEX_API_KEY = os.environ.get("INFLEX_API_KEY", "INFLEX99422128D")

# ðŸŸ¢ Secondary API (New Shruti)
SHRUTI_API_URL = os.environ.get("SHRUTI_API_URL", "https://shrutibots.site")
SHRUTI_API_KEY = os.environ.get("SHRUTI_API_KEY", "ShrutiBotsOjzXBXerA3PCBpdchVLU")
    ...
    try:
        async with aiohttp.ClientSession() as session:
            ...
            async with session.get(
                ...
                if resp.status != 200:
                    LOGGER.error(f"ðŸ”´ {api_name} Error: Status {resp.status}")
                    return None
        ...
        if os.path.exists(file_path) and os.path.getsize(file_path) > 50000:
            LOGGER.info(f"ðŸŸ¢ SOURCE-HOPPING SUCCESS: Downloaded '{title}' from {api_name}!")
            return file_path
        else:
            LOGGER.warning(f"ðŸ”´ {api_name} returned corrupted/empty file for '{title}'. Rejecting it.")
            return None
...
async def ytdl_fallback_download(link: str, download_type: str, title: str = None) -> str:
    ...
    try:
        ...
        if os.path.exists(file_path) and os.path.getsize(file_path) > 50000:
            LOGGER.info(f"ðŸŸ¢ SOURCE-HOPPING SUCCESS: Downloaded '{title}' from yt-dlp!")
            return file_path
        return None
    except Exception as e:
                                ...
                                if song_resp.status == 200:
                                    with open(file_path, "wb") as f:
                                        async for chunk in song_resp.content.iter_chunked(131072):
                                            f.write(chunk)
                                    if os.path.exists(file_path) and os.path.getsize(file_path) > 50000:
                                        LOGGER.info(f"ðŸŸ¢ SOURCE-HOPPING SUCCESS: Downloaded '{clean_title}' from Spotify!")
                                        return file_path
    except Exception as e:
                                ...
                                if song_resp.status == 200:
                                    with open(file_path, "wb") as f:
                                        async for chunk in song_resp.content.iter_chunked(131072):
                                            f.write(chunk)
                                    if os.path.exists(file_path) and os.path.getsize(file_path) > 50000:
                                        LOGGER.info(f"ðŸŸ¢ SOURCE-HOPPING SUCCESS: Downloaded '{clean_title}' from JioSaavn!")
                                        return file_path
    except Exception as e:
...
async def soundcloud_fallback_download(title: str) -> str:
    ...
    try:
        ...
        if os.path.exists(file_path) and os.path.getsize(file_path) > 50000:
            LOGGER.info(f"ðŸŸ¢ SOURCE-HOPPING SUCCESS: Downloaded '{clean_title}' from SoundCloud!")
            return file_path
    except Exception as e:
...
async def download_song(link: str, title: str = None) -> str:
    ...
    LOGGER.warning(f"ðŸ”´ Inflex API failed for '{title}'. Hopping to Shruti API...")
    ...
    LOGGER.warning(f"ðŸ”´ Shruti API failed for '{title}'. Hopping to yt-dlp...")
    ...
    if title:
        LOGGER.warning(f"ðŸ”´ YouTube blocked '{title}'. Hopping to Spotify...")
        sp_result = await spotify_fallback_download(title)
        if sp_result: return sp_result

        LOGGER.warning(f"ðŸ”´ Spotify failed. Hopping to JioSaavn...")
        js_result = await jiosaavn_fallback_download(title)
        if js_result: return js_result

        LOGGER.warning(f"ðŸ”´ JioSaavn failed. Hopping to SoundCloud...")
